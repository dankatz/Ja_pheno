...
nab <- readr::read_csv("C:/Users/danka/Box/texas/NAB/NAB2009_2021_tx_epi_pollen_220831.csv") %>%
  mutate(Cupressaceae_m = cup_all_m) %>% 
  ...
  filter(yr_exp == "yr_2019_2020" | yr_exp == "yr_2020_2021")
  #filter(year > 2018 ) %>% 
...
#compare to raster surfaces of predictions

pheno_preds_NAB <- read_csv("C:/Users/danka/Box/texas/pheno/manual_obs_models/NAB_opening_preds_aprilssm_25km.csv") %>%
  rename(day_exp = day_experiment,
         yr_exp = fs_year) %>%
  dplyr::select(-ID, -lat, -long) %>%
  filter(!is.na(rel_opening)) %>%
  mutate(NAB_station = case_when(NAB_station == "Georgetown" ~ "Austin",
                                 NAB_station != "Georgetown"~ NAB_station)) %>% 
  filter(NAB_station == "Austin" | NAB_station == "San Antonio A" | NAB_station == "San Antonio B"|
...
### NAB data in Texas ############################################################
# load in NAB data 
nab <- readr::read_csv("C:/Users/danka/Box/texas/NAB/NAB_pollen_modeled_linear_interp_201002.csv") %>%
...
pheno_preds_NAB <- read_csv("C:/Users/danka/Box/texas/pheno/manual_obs_models/NAB_opening_preds_jan_dec_prcp_25km.csv") %>%
                    rename(day_exp = day_experiment,
                           yr_exp = fs_year) %>%
                    dplyr::select(-ID, -lat, -long) %>%
                    filter(!is.na(rel_opening)) %>%
...
# load in NAB data 
lb <- readr::read_csv("C:/Users/danka/Box/texas/NASA_project/landon_daily_measurements_2009_2011.csv") %>%
...
pheno_preds_lb <- read_csv("C:/Users/danka/Box/texas/pheno/manual_obs_models/NAB_opening_preds_jan_dec_prcp_25km.csv") %>%
  rename(day_exp = day_experiment,
         yr_exp = fs_year,
         site = NAB_station) %>%
  dplyr::select(-ID, -lat, -long) %>%
